# ============================================================================
# SURVEYS V2 ROUTES — standalone file for the monolithic app.py
# ============================================================================
# Mirrors app_modular/routes/surveys_v2.py but uses the monolithic db/models
# so there are no cross-package import conflicts.
# ============================================================================
...
def register_surveys_v2_routes(app, db):
    """Register all surveys-v2 routes with the Flask app."""

    # Import models - they must be imported after db is initialized in app.py
    # When app.py is run directly it is loaded as '__main__', so we must
    # look there first to avoid re-importing app.py (which would create a
    # second SQLAlchemy instance and cause "not registered" errors).
    import sys
    app_module = sys.modules.get('__main__')
    # Verify the main module is actually our app (has the models we need)
    if not app_module or not hasattr(app_module, 'SurveyV2'):
        app_module = sys.modules.get('app')
    if not app_module or not hasattr(app_module, 'SurveyV2'):
        raise RuntimeError("app module not found - ensure models are defined before calling register_surveys_v2_routes")
    ...
    @app.route('/api/v2/surveys', methods=['GET'])
    def get_surveys_v2():
        try:
    ...
    @app.route('/api/v2/surveys/<int:survey_id>', methods=['GET'])
    def get_survey_v2(survey_id):
    ...
    @app.route('/api/v2/surveys', methods=['POST'])
    def create_survey_v2():
        data = request.json
        name = data.get('name')
        if not name:
    ...
    @app.route('/api/v2/surveys/<int:survey_id>', methods=['PUT'])
    def update_survey_v2(survey_id):
        data = request.json
        survey = SurveyV2.query.get_or_404(survey_id)
    ...
    @app.route('/api/v2/surveys/<int:survey_id>', methods=['DELETE'])
    def delete_survey_v2(survey_id):
        survey = SurveyV2.query.get_or_404(survey_id)
    ...
    @app.route('/api/v2/surveys/<int:survey_id>/duplicate', methods=['POST'])
    def duplicate_survey_v2(survey_id):
        ...
        try:
            ...
            if data.get('copy_organizations', True):
    ...
    @app.route('/api/v2/surveys/<int:survey_id>/organizations', methods=['GET'])
    def get_survey_organizations_v2(survey_id):
        survey = SurveyV2.query.get_or_404(survey_id)
    ...
    @app.route('/api/v2/surveys/<int:survey_id>/organizations', methods=['POST'])
    def attach_organizations_v2(survey_id):
        survey = SurveyV2.query.get_or_404(survey_id)
        data = request.json
    ...
    @app.route('/api/v2/surveys/<int:survey_id>/organizations/<int:org_id>', methods=['DELETE'])
    def detach_organization_v2(survey_id, org_id):
    ...
    @app.route('/api/v2/surveys/<int:survey_id>/sections', methods=['GET'])
    def get_sections_v2(survey_id):
        survey = SurveyV2.query.get_or_404(survey_id)
        return jsonify(survey.sections or []), 200

    @app.route('/api/v2/surveys/<int:survey_id>/sections', methods=['PUT'])
    def update_sections_v2(survey_id):
        survey = SurveyV2.query.get_or_404(survey_id)
        data = request.json
    ...
    logger.info("Surveys V2 routes registered successfully")
